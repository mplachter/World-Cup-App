...
export const $tab = persistedStore<string>('wc2026:tab', 'schedule', (v) =>
  VALID_TABS.includes(v as string),
);
export const $selectedTeam = persistedStore<string | null>(
  'wc2026:selectedTeam',
  null,
  (v) => v === null || typeof v === 'string',
);
export const $showCompleted = persistedStore<boolean>(
  'wc2026:showCompleted',
  false,
  (v) => typeof v === 'boolean',
);
// Per-day collapse state — only collapsed days stored, keeping the blob small.
export const $collapsedDays = persistedStore<Record<string, boolean>>(
  'wc2026:collapsedDays:v1',
  {},
  (v) => v !== null && typeof v === 'object' && !Array.isArray(v),
);
// Schedule filter state — survives tab switches and full rebuilds.
export const $scheduleStageFilter = persistedStore<string>(
  'wc2026:scheduleStageFilter',
  'all',
  (v) => typeof v === 'string',
);
export const $scheduleTeamFilter = persistedStore<string>(
  'wc2026:scheduleTeamFilter',
  '',
  (v) => typeof v === 'string',
);
...
export const espnSummaryCache = persistedCache<EspnDetail>('wc2026:cache:espn:summary:v1', {
  maxEntries: 150,
});
...
export const $squads = createStore<{ loaded: boolean; count: number; error?: boolean }>({
  loaded: false,
  count: 0,
});
