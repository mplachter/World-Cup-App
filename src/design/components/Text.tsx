...
const VARIANT_STYLE: Record<TextVariant, JSX.CSSProperties> = {
  ...
  label:   { fontSize: fontSize.sm,   color: color.textMuted, fontWeight: fontWeight.semibold, textTransform: 'uppercase', letterSpacing: '0.5px' },
...
export function Text({ variant = 'body', as: Tag = 'span', children, style }: TextProps) {
  return (
    <Tag style={{ ...VARIANT_STYLE[variant], ...style }}>
      {children}
    </Tag>
  );
}
