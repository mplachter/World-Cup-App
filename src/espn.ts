...
export async function loadESPN() {
  try {
    ...
    (j.events || []).forEach((ev: Record<string, unknown>) => {
      const comp = (ev.competitions as unknown[]) && (ev.competitions as unknown[])[0] as Record<string, unknown>;
      if (!comp) return;
      const st = comp.status && (comp.status as Record<string, unknown>).type as Record<string, unknown>;
      const state = st ? (st.state as string) : "pre";
      const detail = st ? ((st.shortDetail || st.detail) as string) : "";
      const clock = comp.status ? ((comp.status as Record<string, unknown>).displayClock as string) : "";
      ...
      const home = (teams.find((t: unknown) => (t as Record<string, unknown>).homeAway === "home") || teams[0]) as Record<string, unknown>;
      const away = (teams.find((t: unknown) => (t as Record<string, unknown>).homeAway === "away") || teams[1]) as Record<string, unknown>;
      ...
      map[key] = {
        ...
        homeScore: home.score as string, awayScore: away.score as string,
...
function buildAthleteMap(j: Record<string, unknown>): Record<string, string> {
  ...
  const teams = (j && j.boxscore && (j.boxscore as Record<string, unknown>).players as unknown[]) || [];
  (teams as Record<string, unknown>[]).forEach(t => {
    ...
    groups.forEach(g => {
      ...
      arr.forEach(p => { addAthlete((p.athlete || p) as Record<string, unknown>); });
    });
  });
  const rosters = (j && j.rosters as unknown[]) || [];
  (rosters as Record<string, unknown>[]).forEach(r => {
    ((r.roster || r.athletes || []) as Record<string, unknown>[]).forEach(p => addAthlete((p.athlete || p) as Record<string, unknown>));
  });
  return map;
}

function athleteNameFromInvolved(a: Record<string, unknown> | null, athleteMap: Record<string, string>): string {
  if (!a) return '';
  const athlete = a.athlete as Record<string, unknown> | undefined;
...
function parseEventText(text: string, evType: string): { playerIn?: string; playerOut?: string; player?: string; scorer?: string; ownGoal?: boolean } | null {
  if (!text) return null;
...
function parseEspnSummary(j: Record<string, unknown>) {
  const subs: unknown[] = []; const cards: unknown[] = []; const goals: unknown[] = [];
  ...
  if (typeof window !== 'undefined' && !window.__WC_ESPN_DUMPED && window.__WC_DEBUG !== false) {
    ...
    console.log('[WC ESPN dump] boxscore keys:', j && j.boxscore && Object.keys(j.boxscore as object));
  ...
  (events as Record<string, unknown>[]).forEach(ev => {
    const t = (((ev.type && (ev.type as Record<string, unknown>).text) || ev.text || '') as string).toLowerCase();
    const minute = ((ev.clock && (ev.clock as Record<string, unknown>).displayValue) || ev.minute || '') as string;
    const teamName = (ev.team && ((ev.team as Record<string, unknown>).displayName || (ev.team as Record<string, unknown>).name) || '') as string;
    const teamShort = (ev.team && ((ev.team as Record<string, unknown>).abbreviation || (ev.team as Record<string, unknown>).shortDisplayName) || teamName) as string;
    const involved = (ev.athletesInvolved || ev.athletes || ev.participants || []) as Record<string, unknown>[];
...
function parseEspnLineups(j: Record<string, unknown>, athleteMap: Record<string, string>) {
  ...
  (rosters as Record<string, unknown>[]).forEach(r => {
    if (!r) return;
    const teamName = (r.team && ((r.team as Record<string, unknown>).displayName || (r.team as Record<string, unknown>).name) || '') as string;
    const homeAway = ((r.homeAway || '') as string).toLowerCase();
    const formation = (r.formation && ((r.formation as Record<string, unknown>).name || (r.formation as Record<string, unknown>).text))
...
export async function loadEspnSummary(eventId: string | null, force = false) {
  ...
  if (!existing && !force) {
    ...
    if (cached) {
      $espnDetails.set({ ...cur, [eventId]: {
        subs: cached.subs || [], cards: cached.cards || [], goals: cached.goals || [], lineups: cached.lineups || { home: null, away: null },
  ...
  $espnDetails.set({ ...$espnDetails.get(), [eventId]: { ...(existing || { subs: [], cards: [], goals: [], lineups: { home: null, away: null } }), loading: true } } as Record<string, EspnDetail>);
  try {
    ...
    const statusType = (j && j.header && (j.header as Record<string, unknown>).competitions
                       && ((j.header as Record<string, unknown>).competitions as unknown[])[0]
                       && (((j.header as Record<string, unknown>).competitions as unknown[])[0] as Record<string, unknown>).status
                       && ((((j.header as Record<string, unknown>).competitions as unknown[])[0] as Record<string, unknown>).status as Record<string, unknown>).type)
                       || (j && j.gameInfo && (j.gameInfo as Record<string, unknown>).status && ((j.gameInfo as Record<string, unknown>).status as Record<string, unknown>).type)
                       || null;
    const isFinal = !!(statusType && ((statusType as Record<string, unknown>).completed === true || (statusType as Record<string, unknown>).state === 'post'));
    ...
    $espnDetails.set({ ...cur2, [eventId]: { ...(cur2[eventId] || { subs: [], cards: [], goals: [], lineups: { home: null, away: null } }), loading: false, error: msg } } as Record<string, EspnDetail>);
  }
}
