...
function saveCachedSim(payload: unknown) {
  try {
    localStorage.setItem(SIM_LS_KEY, JSON.stringify(payload));
  } catch {}
}
...
(function hydrateSim() {
  ...
  if (cached) {
    $sim.set({
      status: 'done',
      data: cached.data,
      trials: cached.trials || SIM_TRIALS_DEFAULT,
      completed: cached.trials || SIM_TRIALS_DEFAULT,
...
export function isClinchedTop2(
  team: string,
  group: string,
  filteredByKey: Record<string, Match>,
): boolean {
  ...
  std.forEach((r) => {
    remaining[r.team] = totalGames - r.mp;
  });
...
export function wasAlreadyQualifiedBefore(
  team: string,
  group: string,
  ...
  Object.entries(byKey).forEach(([k, e]) => {
    if (e.stage === 'group' && e.group === group && e.score && e.date && e.date < beforeDate)
      filtered[k] = e;
  });
  ...
  Object.entries(byTeam).forEach(([team, matches]) => {
    ...
    recent.forEach((m, i) => {
      ...
      if (
        m.stage === 'group' &&
        m.group &&
        wasAlreadyQualifiedBefore(team, m.group, byKey, m.date)
      ) {
...
export function combineSuspensionPenalty(players: { rank: number | null; pos?: string }[]): number {
  ...
  const total = players.reduce(
    (sum, p) => sum + suspensionBasePenalty(p.rank) * suspensionPositionMultiplier(p.pos),
    0,
  );
...
function suspensionPenaltyForTeam(team: string, date: string, stage: string): number {
  ...
  return combineSuspensionPenalty(
    suspended.map((s) => ({
...
export async function startSimulation(
  ...
  await Promise.all(
    Object.keys(TEAM_GROUP).map(async (team) => {
      ...
      try {
        await prefetchSuspensionData(team, team, date);
      } catch {
        /* best-effort: missing data just means no penalty */
      }
  ...
  for (let trial = 0; trial < trials; trial++) {
    ...
    for (const p of allGroupPairs) {
      if (p.existing) {
        simByKey[p.key] = p.existing;
      } else {
        const dH =
          (formDelta[p.home] || 0) +
          (suspensionGroupKey[p.home] === p.key ? suspensionDelta[p.home] : 0);
        const dA =
          (formDelta[p.away] || 0) +
          (suspensionGroupKey[p.away] === p.key ? suspensionDelta[p.away] : 0);
    ...
    const thirds = GRPS.map((g) => {
      ...
      return row
        ? {
            ...
            gd: row.gf - row.ga,
            gf: row.gf,
            fifa: FIFA[row.team] || 99,
          }
        : null;
    })
      .filter((x): x is NonNullable<typeof x> => x !== null)
      .sort((a, b) => b.pts - a.pts || b.gd - a.gd || b.gf - a.gf || a.fifa - b.fifa);
    ...
    for (const s of R32_STRUCTURE) {
      ...
      const dH =
        (formDelta[hT || ''] || 0) +
        (hT && suspensionKnockoutTeams.has(hT) ? suspensionDelta[hT] : 0);
      const dA =
        (formDelta[aT || ''] || 0) +
        (aT && suspensionKnockoutTeams.has(aT) ? suspensionDelta[aT] : 0);
    ...
    const sf1 = slotTeam[BRACKET_THIRD_PLACE_FEEDS.home],
      sf2 = slotTeam[BRACKET_THIRD_PLACE_FEEDS.away];
    const loser1 =
      sf1 && winnerOf[BRACKET_THIRD_PLACE_FEEDS.home]
        ? winnerOf[BRACKET_THIRD_PLACE_FEEDS.home] === sf1.home
          ? sf1.away
          : sf1.home
        : undefined;
    const loser2 =
      sf2 && winnerOf[BRACKET_THIRD_PLACE_FEEDS.away]
        ? winnerOf[BRACKET_THIRD_PLACE_FEEDS.away] === sf2.home
          ? sf2.away
          : sf2.home
        : undefined;
  ...
  Object.entries(counts).forEach(([num, sides]) => {
    out[num] = {
      home: Object.entries(sides.home)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .map(([team, c]) => ({ team, pct: c / trials })),
      away: Object.entries(sides.away)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .map(([team, c]) => ({ team, pct: c / trials })),
    };
  ...
  let changed = true;
  let safety = 0;
  const isHardLocked = (cands: { team: string; pct: number }[]) =>
    cands && cands.length && cands[0].pct >= 0.9;
...
export function projectBracket(byKey: Record<string, Match>, _mode: string) {
  ...
  GRPS.forEach((g) => {
    standings[g] = calcStandings(g, byKey);
  });
  ...
  const thirds = GRPS.map((g) => {
    ...
    .filter((t): t is NonNullable<typeof t> => t !== null && !!t.team)
    .sort((a, b) => b.pts - a.pts || b.gf - b.ga - (a.gf - a.ga) || b.gf - a.gf || a.fifa - b.fifa);

  const top8 = thirds.slice(0, 8);
  const top8Groups = top8.map((t) => t.group);
  const groupComplete = GRPS.every((g) => standings[g].every((r) => r.mp === 3));
  const thirdsConfidence = groupComplete
    ? 'projected'
    : thirds.length >= 8
      ? 'projected'
      : 'unknown';
  ...
  const r32 = [
    {
      num: 73,
      home: teamSlot(RU['A'], RUCONF['A']),
      away: teamSlot(RU['B'], RUCONF['B']),
      label: 'M73',
    },
    { num: 74, home: teamSlot(W['E'], WCONF['E']), away: thirdSlotFor(74), label: 'M74' },
    {
      num: 75,
      home: teamSlot(W['F'], WCONF['F']),
      away: teamSlot(RU['C'], RUCONF['C']),
      label: 'M75',
    },
    {
      num: 76,
      home: teamSlot(W['C'], WCONF['C']),
      away: teamSlot(RU['F'], RUCONF['F']),
      label: 'M76',
    },
    { num: 77, home: teamSlot(W['I'], WCONF['I']), away: thirdSlotFor(77), label: 'M77' },
    {
      num: 78,
      home: teamSlot(RU['E'], RUCONF['E']),
      away: teamSlot(RU['I'], RUCONF['I']),
      label: 'M78',
    },
    ...
    {
      num: 83,
      home: teamSlot(RU['K'], RUCONF['K']),
      away: teamSlot(RU['L'], RUCONF['L']),
      label: 'M83',
    },
    {
      num: 84,
      home: teamSlot(W['H'], WCONF['H']),
      away: teamSlot(RU['J'], RUCONF['J']),
      label: 'M84',
    },
    { num: 85, home: teamSlot(W['B'], WCONF['B']), away: thirdSlotFor(85), label: 'M85' },
    {
      num: 86,
      home: teamSlot(W['J'], WCONF['J']),
      away: teamSlot(RU['H'], RUCONF['H']),
      label: 'M86',
    },
    { num: 87, home: teamSlot(W['K'], WCONF['K']), away: thirdSlotFor(87), label: 'M87' },
    {
      num: 88,
      home: teamSlot(RU['D'], RUCONF['D']),
      away: teamSlot(RU['G'], RUCONF['G']),
      label: 'M88',
    },
  ];
...
// ─── MATH LOCK ────────────────────────────────────────────────────────────────
const _mathLockCache: {
  byKeyFingerprint: string;
  locks: Record<string, { group: string; position: number }>;
} = { byKeyFingerprint: '', locks: {} };
export function getMathLocks(
  byKey: Record<string, Match>,
): Record<string, { group: string; position: number }> {
  ...
  GRPS.forEach((g) => {
    ...
    if (groupFinished) {
      std.forEach((r, idx) => {
        locks[r.team] = { group: g, position: idx };
      });
      return;
    }
    std.forEach((r, idx) => {
