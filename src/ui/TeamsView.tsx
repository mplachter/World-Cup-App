...
function teamR32Distribution(team: string) {
  const data = $sim.get().data as Record<
    string,
...
function TeamDetailView({ team }: TeamDetailProps) {
  ...
  const simData = simState.data as Record<
    string,
    { home: { team: string; pct: number }[]; away: { team: string; pct: number }[] }
  > | null;
    ...
    <div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff', marginBottom: '2px' }}>
            {team}
          </div>
          ...
          {rows.map((r, i) => (
            <div
              ...
              style={{
                ...
                padding: '5px 10px',
                borderRadius: '4px',
                background: r.team === team ? 'rgba(59,130,246,0.12)' : 'rgba(255,255,255,0.02)',
                marginBottom: '2px',
                border:
                  r.team === team ? '1px solid rgba(59,130,246,0.3)' : '1px solid transparent',
              }}
            >
              <span
                style={{
                  fontSize: '11px',
                  color: i < 2 ? '#3b82f6' : i === 2 ? '#f59e0b' : '#6b7280',
              ...
              <span
                style={{
                  ...
                  color: '#e2e8f0',
                  flex: 1,
                  fontWeight: r.team === team ? 600 : 400,
                }}
              >
                {r.team}
              </span>
            ...
            >
              ...
              {dist.slice(0, isLocked ? 1 : 5).map((d) => {
                ...
                const dateStr = entry?.date
                  ...
                  : '';
                const pctTxt =
                  d.pct >= 0.995 ? '>99%' : d.pct < 0.005 ? '<1%' : Math.round(d.pct * 100) + '%';
                return (
                  <div
                    key={d.matchNum + d.side}
                    style={{
                      padding: '8px 10px',
                      marginBottom: '6px',
                      borderRadius: '6px',
                    ...
                    >
                      <span
                        style={{
                          fontSize: '11px',
                          fontWeight: 600,
                          color: isLocked ? '#4ade80' : '#93c5fd',
                        }}
                      >
                        {isLocked ? '✓ Confirmed slot' : pctTxt + ' chance'}
                      </span>
                      ...
                      {entry?.ground && (
                        <span style={{ fontSize: '10px', color: '#374151' }}>· {entry.ground}</span>
                      )}
                    </div>
                    {opponents.length > 0 && (
                      <div
                      ...
                      >
                        ...
                        {opponents.slice(0, 3).map((o, i) => (
                          <span
                            ...
                            onClick={() => navigateToTeam(o.team)}
                          >
                            {i > 0 ? ', ' : ''}
                            {FLAG[o.team] || ''} {o.team} {Math.round(o.pct * 100)}%
                          </span>
                        ))}
                      </div>
                    )}
        ...
        {myMatches.map((m) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontSize: '10px', color: '#6b7280', minWidth: '60px', flexShrink: 0 }}>
                {dateStr}
              </span>
              <span style={{ fontSize: '10px', color: '#6b7280', minWidth: '70px', flexShrink: 0 }}>
                {m.time || ''}
              </span>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '16px' }}>
        {LEAGUES.map((l) => (
          <button
            key={l}
            onClick={() => {
              setLeagueFilter(leagueFilter === l ? null : l);
              setSearch('');
            }}
            style={{
              padding: '4px 8px',
          ...
          return (
            <div
              ...
              onMouseOver={(e) => {
                ...
                (e.currentTarget as HTMLElement).style.border = '1px solid rgba(59,130,246,0.25)';
              }}
              onMouseOut={(e) => {
              ...
              >
                <span
                  style={{
                    fontSize: '11px',
                    fontFamily: 'monospace',
                    color: rank <= 5 ? '#fbbf24' : rank <= 15 ? '#94a3b8' : '#4b5563',
                    width: '20px',
                    textAlign: 'right',
                    flexShrink: 0,
                  }}
                ...
                {sq ? (
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <HTMLEl factory={() => leagueBar(getLC(sq), sq.length)} />
                  </div>
                ) : (
                  <span
                    style={{ flex: 1, fontSize: '11px', color: '#4b5563', fontStyle: 'italic' }}
                  >
                    {squadsState.loaded ? 'no data' : 'loading…'}
                  </span>
                )}
                <span
                  style={{ fontSize: '10px', color: '#4b5563', flexShrink: 0, marginLeft: '4px' }}
                >
                  ›
                </span>
              </div>
