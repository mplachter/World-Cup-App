import { useState, useEffect as preactUseEffect, useRef } from 'preact/hooks';
import type { Match } from '../types';
import { div, span, btn } from '../dom';
import { FIFA, FLAG, pkey, parseScore, norm, espnNorm } from '../constants';
import { $data, $espn, $espnDetails, $selectedTeam, $tab, navigateToTeam, clickableTeam } from '../state';
...
function resolveDisplay(
  ...
  const allLocked =
    cands.length >= 2 && cands.every(c => mathLocks[c.team] && c.pct >= 0.02);
  if (locked)
    return {
      display: top.team,
      isPredicted: true,
...
function MatchEvents({ espnDetail, home, away, goals1, goals2 }: { espnDetail: any; home: string; away: string; goals1: any[]; goals2: any[] }) {
  if (!espnDetail) return <div style={{ fontSize: '11px', color: '#4b5563', fontStyle: 'italic' }}>Loading match events…</div>;
  if (espnDetail.loading && !espnDetail.loaded) return <div style={{ fontSize: '11px', color: '#4b5563', fontStyle: 'italic' }}>Loading match events…</div>;
  if (espnDetail.error && !espnDetail.loaded) return <div style={{ fontSize: '11px', color: '#7f1d1d' }}>Could not load match events ({espnDetail.error})</div>;
  ...
  if (!subs.length && !cards.length && !showEspnGoals && !hasLineups) {
    return <div style={{ fontSize: '11px', color: '#4b5563', fontStyle: 'italic', marginBottom: '12px' }}>No events yet.</div>;
  }
  ...
  if (homeLU && homeSide === 'away') { const tmp = homeLU; homeLU = awayLU; awayLU = tmp; }
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      {showEspnGoals && (() => {
        ...
        return (
          <>
            <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.5px', marginBottom: '4px', textTransform: 'uppercase' }}>Goals</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
              <div>{hg.map((g: any, i: number) => <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.8' }}>{g.ownGoal ? '⚽ OG ' : g.penalty ? '⚽ P ' : '⚽ '}{g.scorer || '?'} <span style={{ color: '#4b5563' }}>{g.minute}</span></div>)}</div>
              <div style={{ textAlign: 'right' }}>{ag.map((g: any, i: number) => <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.8' }}><span style={{ color: '#4b5563' }}>{g.minute}</span> {g.scorer || '?'} {g.ownGoal ? 'OG ⚽' : g.penalty ? 'P ⚽' : '⚽'}</div>)}</div>
            </div>
          </>
        );
      })()}
      {subs.length > 0 && (() => {
        ...
        return (
          <>
            <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.5px', marginBottom: '4px', textTransform: 'uppercase' }}>Substitutions</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
              <div>{hs.map((s: any, i: number) => <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.7' }}>🔄 <span style={{ color: '#10b981' }}>{s.playerIn || '?'}</span> ← <span style={{ color: '#9ca3af' }}>{s.playerOut || '?'}</span> <span style={{ color: '#4b5563' }}>{s.minute}</span></div>)}</div>
              <div style={{ textAlign: 'right' }}>{as_.map((s: any, i: number) => <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.7' }}><span style={{ color: '#4b5563' }}>{s.minute}</span> <span style={{ color: '#9ca3af' }}>{s.playerOut || '?'}</span> → <span style={{ color: '#10b981' }}>{s.playerIn || '?'}</span> 🔄</div>)}</div>
            </div>
          </>
        );
      })()}
      {cards.length > 0 && (() => {
        ...
        return (
          <>
            <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.5px', marginBottom: '4px', textTransform: 'uppercase' }}>Cards</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
              <div>{hc.map((c: any, i: number) => <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.7' }}>{c.type === 'red' ? '🟥' : c.type === 'yellowred' ? '🟨🟥' : '🟨'} {c.player || '?'} <span style={{ color: '#4b5563' }}>{c.minute}</span></div>)}</div>
              <div style={{ textAlign: 'right' }}>{ac.map((c: any, i: number) => <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.7' }}><span style={{ color: '#4b5563' }}>{c.minute}</span> {c.player || '?'} {c.type === 'red' ? '🟥' : c.type === 'yellowred' ? '🟨🟥' : '🟨'}</div>)}</div>
            </div>
          </>
        );
      })()}
      {hasLineups && (() => {
        ...
        function lineupCol(lu: any, align: string) {
          if (!lu) return <div style={{ fontSize: '11px', color: '#4b5563', fontStyle: 'italic', textAlign: align as any }}>—</div>;
          return <div>{bucketize(lu.starters).map((p: any, i: number) => (
            <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.7', textAlign: align as any }}>
              {align === 'right'
                ? <>{p.position && <span style={{ color: '#4b5563', fontSize: '10px', marginRight: '4px' }}>{p.position}</span>}{p.name || '?'}{p.jersey && <span style={{ color: '#4b5563', marginLeft: '4px' }}>#{p.jersey}</span>}</>
                : <>{p.jersey && <span style={{ color: '#4b5563', marginRight: '4px' }}>#{p.jersey}</span>}{p.name || '?'}{p.position && <span style={{ color: '#4b5563', fontSize: '10px', marginLeft: '4px' }}>{p.position}</span>}</>
              }
            </div>
          ))}</div>;
        }
        return (
          <>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '4px' }}>
              <span style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase' }}>Starting XI</span>
              {(homeFmt || awayFmt) && <span style={{ fontSize: '10px', color: '#4b5563' }}>{homeFmt || '?'} · {awayFmt || '?'}</span>}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
...
export function MatchCard({ entry, todayISO, showSquads = true }: MatchCardProps) {
  ...
  const sH = isUpcoming ? computeSuspensions(homeD.display, date, stage) : { suspended: [], onYellow: [] };
  const sA = isUpcoming ? computeSuspensions(awayD.display, date, stage) : { suspended: [], onYellow: [] };
    ...
    >
      ...
      <div style={{ cursor: 'pointer' }} onClick={() => setIsOpen(!isOpen)}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px', background: barBg, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '11px', fontWeight: '700', color: sColor }}>{sLabel}{isLive && clock ? ' ' + clock : ''}</span>
            {dateStr && <span style={{ fontSize: '11px', color: '#6b7280' }}>{dateStr}</span>}
            {!isLive && time && <span style={{ fontSize: '11px', color: '#9ca3af' }}>{time}</span>}
            {broadcast && <span style={{ fontSize: '10px', color: '#4b5563', background: 'rgba(255,255,255,0.05)', padding: '1px 5px', borderRadius: '3px' }}>{broadcast}</span>}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '11px', color: '#374151', maxWidth: '90px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ground || round || ''}</span>
            {scoreDisp && (
          ...
          >
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', minWidth: 0 }}>
                <span style={{ fontSize: '14px', fontWeight: hWon ? '700' : '500', color: hWon ? '#fff' : '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{homeD.display}</span>
                {homeD.isPredicted && <span style={{ fontSize: '8px', color: '#a8a29e', fontFamily: 'monospace', flexShrink: 0, background: 'rgba(168,162,158,0.12)', padding: '1px 4px', borderRadius: '3px', letterSpacing: '0.3px' }}>PRED</span>}
              </div>
              {r1 && <div style={{ fontSize: '10px', color: '#6b7280' }}>FIFA #{r1}</div>}
              {homeD.alternates.length > 0 && <div style={{ fontSize: '9px', color: '#78716c', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>or {homeD.alternates.map(t => (FLAG[t] || '') + ' ' + t).join(', ')}</div>}
            </div>
          </div>

          <div style={{ fontSize: '11px', fontWeight: '700', color: '#4b5563', padding: '0 4px' }}>VS</div>
          ...
          >
            <div style={{ minWidth: 0, textAlign: 'right' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', minWidth: 0, justifyContent: 'flex-end' }}>
                {awayD.isPredicted && <span style={{ fontSize: '8px', color: '#a8a29e', fontFamily: 'monospace', flexShrink: 0, background: 'rgba(168,162,158,0.12)', padding: '1px 4px', borderRadius: '3px', letterSpacing: '0.3px' }}>PRED</span>}
                <span style={{ fontSize: '14px', fontWeight: aWon ? '700' : '500', color: aWon ? '#fff' : '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{awayD.display}</span>
              </div>
              {r2 && <div style={{ fontSize: '10px', color: '#6b7280' }}>FIFA #{r2}</div>}
              {awayD.alternates.length > 0 && <div style={{ fontSize: '9px', color: '#78716c', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>or {awayD.alternates.map(t => (FLAG[t] || '') + ' ' + t).join(', ')}</div>}
            </div>
      ...
      {isOpen && (
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '12px' }}>
          ...
          {(goals1.length > 0 || goals2.length > 0) && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
              <div>
                {goals1.map((g, i) => (
                  <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.8' }}>
                    {(g as any).owngoal ? '⚽ OG ' : (g as any).penalty ? '⚽ P ' : '⚽ '}{g.name} <span style={{ color: '#4b5563' }}>{String(g.minute)}'</span>
                  </div>
                ))}
              </div>
              <div style={{ textAlign: 'right' }}>
                {goals2.map((g, i) => (
                  <div key={i} style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.8' }}>
                    <span style={{ color: '#4b5563' }}>{String(g.minute)}'</span> {g.name} {(g as any).owngoal ? 'OG ⚽' : (g as any).penalty ? 'P ⚽' : '⚽'}
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ESPN events */}
          {canHaveEvents && <MatchEvents espnDetail={espnDetail} home={home} away={away} goals1={goals1} goals2={goals2} />}

          {/* Suspensions */}
          {isUpcoming && (sH.suspended.length > 0 || sA.suspended.length > 0 || sH.onYellow.length > 0 || sA.onYellow.length > 0) && (
            <div style={{ marginBottom: '12px' }}>
              {(sH.suspended.length > 0 || sA.suspended.length > 0) && (
                <>
                  <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.5px', marginBottom: '4px', textTransform: 'uppercase' }}>Suspensions</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                    <div>
                      {sH.suspended.map((p, i) => (
                        <div key={i} style={{ fontSize: '11px', color: '#fca5a5', lineHeight: '1.7' }}>
              ...
              {(sH.onYellow.length > 0 || sA.onYellow.length > 0) && (
                <>
                  <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.5px', marginBottom: '4px', textTransform: 'uppercase' }}>On a yellow</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                    <div>
                      {sH.onYellow.map((p, i) => (
                        <div key={i} style={{ fontSize: '11px', color: '#fcd34d', lineHeight: '1.7' }}>
          ...
          {showSquads && (() => {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '8px' }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: 600, color: '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{homeD.display}</span>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: 600, color: '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{awayD.display}</span>
            ...
            return (
              <>
                <div
                  style={{ marginTop: '8px', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '11px', color: '#9ca3af', userSelect: 'none' }}
                  onClick={(e) => { e.stopPropagation(); setSquadsOpen(!squadsOpen); }}
                >
                  <span>Squads</span>
                  <span style={{ fontSize: '10px', color: '#6b7280', transform: squadsOpen ? 'rotate(180deg)' : 'rotate(0)', display: 'inline-block', transition: 'transform .15s' }}>▼</span>
                </div>
...
// Legacy factory function (for backward compat with groups.ts)
