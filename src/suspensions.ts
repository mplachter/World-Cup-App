...
export function getTeamPriorMatches(team: string, beforeDate: string) {
  const all = ($data.get() || { all: [] as unknown[] }).all as {
    home: string;
    away: string;
    date: string;
    time: string;
...
export function computeSuspensions(team: string, upcomingDate: string, upcomingStage: string) {
  ...
  const players: Record<
    string,
    { origName: string; yellows: number; suspendedNext: boolean; reason: string | null }
  > = {};
  ...
  priorMatches.forEach((entry) => {
    ...
    if (!eventId) return;
    const det = ($espnDetails.get() as Record<string, unknown>)[eventId] as
      { loaded?: boolean; cards?: { player?: string; type?: string; team?: string }[] } | undefined;
    ...
    cards.forEach((c) => {
      ...
      if (!players[nk])
        players[nk] = { origName: c.player, yellows: 0, suspendedNext: false, reason: null };
      const p = players[nk];
      if (c.type === 'red') {
        p.suspendedNext = true;
        p.reason = 'red card';
      } else if (c.type === 'yellowred') {
        p.suspendedNext = true;
        p.reason = '2 yellows (same match)';
...
export async function prefetchSuspensionData(
  homeTeam: string,
  awayTeam: string,
  upcomingDate: string,
) {
