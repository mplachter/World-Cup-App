...
_RAW_PLAYER_RANKS.forEach(([r, n]) => { PLAYER_RANK[normName(n)] = r; });
export function getPlayerRank(name: string): number | null { return PLAYER_RANK[normName(name)] || null; }

// ─── LEAGUES ─────────────────────────────────────────────────────────────────

export const LEAGUES = ["Premier League","La Liga","Bundesliga","Ligue 1","Serie A","Primeira Liga","Eredivisie","Saudi Pro League","MLS","Süper Lig","Scottish Prem","Other"];
export const LC: Record<string, string> = {"Premier League":"#4c1d95","La Liga":"#991b1b","Bundesliga":"#92400e","Ligue 1":"#1e3a5f","Serie A":"#14532d","Primeira Liga":"#065f46","Eredivisie":"#6d28d9","Saudi Pro League":"#78350f","MLS":"#1d4ed8","Süper Lig":"#be123c","Scottish Prem":"#1e40af","Other":"#292524"};
...
export function espnNorm(n: string): string { return ESPN_NORM[n] || n; }

// ─── NAME / KEY UTILITIES ─────────────────────────────────────────────────────

// Openfootball match team-name normalization
export const NORM: Record<string, string> = {"Bosnia & Herzegovina":"Bosnia and Herzegovina","United States":"USA","Côte d'Ivoire":"Ivory Coast","Korea Republic":"South Korea"};
...
export function processRaw(json: { matches?: unknown[] }): MatchData {
  const byKey: Record<string, Match> = {}, byNum: Record<string, Match> = {}, all: Match[] = [];
  (json.matches || []).forEach((m: unknown) => {
    const raw = m as Record<string, unknown>;
    const h = norm(raw.team1 as string), a = norm(raw.team2 as string);
...
export function getLC(sq: Player[]): Record<string, number> {
  ...
  sq.forEach(p => { c[p.league] = (c[p.league] || 0) + 1; });
  return c;
}
...
export function calcStandings(gk: string, byKey: Record<string, Match>): Standing[] {
  ...
  teams.forEach(t => { rows[t] = {team:t,mp:0,w:0,d:0,l:0,gf:0,ga:0,pts:0}; });
  for (let i = 0; i < teams.length; i++) {
    for (let j = i + 1; j < teams.length; j++) {
      ...
      if (!sc) continue;
      const { h, a } = sc;
      const hr = rows[e.home], ar = rows[e.away];
      if (!hr || !ar) continue;
      hr.mp++; ar.mp++; hr.gf += h; hr.ga += a; ar.gf += a; ar.ga += h;
      if (h > a) { hr.w++; hr.pts += 3; ar.l++; }
      else if (h < a) { ar.w++; ar.pts += 3; hr.l++; }
